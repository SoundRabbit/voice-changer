...
int main(){
	...
	overlapAdd<double>(
		...
		[&](auto sampleBegin, auto sampleEnd){			// 窓関数がかかった波形への処理内容
			...
			for(int k=0; k<originalFrequencySpectrum.size(); k+=2){
				originalFrequencySpectrum[k] = pow(resultOfRdft[k], 2) + pow(resultOfRdft[k+1], 2);
			}

			// 取得したスペクトルにLPFをかける
			vector<double> frequencySpectrum(originalFrequencySpectrum.begin(), originalFrequencySpectrum.end());
			rdft(n, -1, frequencySpectrum.data(), ip, w);
			...
			// 1オクターブ上げる
			for(int k=0; k<frequencySpectrum.size(); k+=2) {
				frequencySpectrum[k*2] = frequencySpectrum[k]
			}

			// 編集されたスペクトルを反映
			for(int k=0; k<frequencySpectrum.size(); k+=2){
				resultOfRdft[k] *= frequencySpectrum[k] / originalFrequencySpectrum[k];
				resultOfRdft[k+1] *= frequencySpectrum[k] / originalFrequencySpectrum[k];
			}
