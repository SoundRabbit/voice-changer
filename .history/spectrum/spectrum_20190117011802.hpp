...
template<std::size_t length>
template<typename FowardAccessIterator>
Spectrum<length>::Spectrum(FowardAccessIterator begin, FowardAccessIterator end) {
	{
		auto itr = begin;
	...
	cdft(length, -1, resultOfCdft.data(), ip, w);
}

template<std::size_t length>
Spectrum<length>::Intensity Spectrum<length>::operator[] (std::size_t i){
	int k1 = 2*i;
	int k2 = length*2 - i - 1;
	return Spectrum<length>::Intensity(resultOfCdft[k1], resultOfCdft[k1+1], resultOfCdft[k2], resultOfCdft[k2+1]);
}
