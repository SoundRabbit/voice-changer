...
int main(){
	...
	overlapAdd<double>(
		...
		[&](auto sampleBegin, auto sampleEnd){			// 窓関数がかかった波形への処理内容
			...
			// 取得したスペクトルにLPFをかける
			vector<double> frequencySpectrum(originalSpectrum.begin(), originalSpectrum.end());
			...
			// 編集されたスペクトルを反映
			for(int k=0; k<frequencySpectrum.size(); k+=2){
				resultOfRdft[k] *= frequencySpectrum[k] / originalSpectrum[k];
				resultOfRdft[k+1] *= frequencySpectrum[k] / originalSpectrum[k];
			}
