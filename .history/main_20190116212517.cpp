...
int main(){
	...
	overlapAdd<double>(
		...
		[&](auto first, auto last){			// 窓関数がかかった波形への処理内容
			...
			vector<double> frequencySpectrum(resultOfRdft.size()/2 + 1, 0);
			rdft(n, -1, resultOfRdft.data(), ip, w);

			// c.f. fftsg.c line from 96 to 99
			frequencySpectrum[0] = resultOfRdft[0];
			for(int i=1; i<frequencySpectrum.size(); i++){
				frequencySpectrum[i] = pow(resultOfRdft[i*2], 2) + pow(resultOfRdft[i*2+1], 2);
			}
