...
int main(){
	...
	overlapAdd<double>(
		...
		[&](auto sampleBegin, auto sampleEnd){			// 窓関数がかかった波形への処理内容
			...
			WavFile test;
			test.quantifyingBitNum = wav.quantifyingBitNum;
			test.samplingRate = wav.samplingRate;
			for(int i=0; i<originalSpectrum.size(); i++){
				test.data.push_back(originalSpectrum.get(i));
			}
			test.save(std::string("test-") + testCounter);
